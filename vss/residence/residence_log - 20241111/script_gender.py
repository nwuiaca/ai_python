...
october_1_gender_ratios = {
    year: data.groupby("CAMPUS_NAME")["GENDER"].value_counts(normalize=True).unstack(fill_value=0) * 100
    for year, data in october_1_data.items()
}

# Combine racial data across years into a single DataFrame for plotting trends
combined_gender_ratios_over_years = pd.concat(october_1_gender_ratios, axis=0)
combined_gender_ratios_over_years.index.names = ['Year', 'Campus']

# Plot racial trends by campus across years
for campus in combined_gender_ratios_over_years.index.get_level_values('Campus').unique():
    campus_data = combined_gender_ratios_over_years.xs(campus, level='Campus')

    plt.figure(figsize=(10, 6))
    for gender in campus_data.columns:
        plt.plot(campus_data.index, campus_data[gender], marker='o', label=gender)

    # Chart details
    plt.title(f"Gender Trends for {campus} Campus Over the Years")
    plt.xlabel("Year")
